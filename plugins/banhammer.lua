    ...
    if vusername == member then
      ...
      if get_cmd == 'kick' then
        if is_momod2(member_id, chat_id) then
          return send_large_msg(receiver, "شما نمیتوانید ادمین های ربات یا گروه را پاک کنید!")
        end
        return kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) then
          return send_large_msg(receiver, "شما نمیتوانید ادمین های ربات یا گروه را بن کنید!")
        end
        send_large_msg(receiver, 'یوزر @'..member..' ['..member_id..'] با موفقیت بن شد')
        return ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, 'یوزر @'..member..' ['..member_id..'] با موفقیت آنبن شد')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'یوزر @'..member..' ['..member_id..'] از همه گروه ها بن شد')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'یوزر @'..member..' ['..member_id..'] از همه گروه ها آنبن شد')
    ...
    if msg.to.type == 'chat' then
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        if not is_admin(msg) and is_momod2(tonumber(matches[2]), msg.to.id) then
          return "شما نمیتوانید ادمین های ربات یا گروه را بن کنید!"
        end
    ...
    if msg.to.type == 'chat' then
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
          return "شما نمیتوانید ادمین های گروه یا ربات را بن کنید!"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
